        ...
        <td>
            ...
            <a href=''>
                <xsl:attribute name="onclick">
					...
					//location.href='ck68/index.html?<xsl:value-of select="name"/>';
					//window.open ('ck68/index.html?<xsl:value-of select="name"/>');
          				var url='ck68/index.html?<xsl:value-of select="name"/>';                             //转向网页的地址; 
           				var name='playwindow';                            //网页名称，可为空; 
						...
						//if (url.indexOf("?") != -1) { alert(url.substring(0,url.lastIndexOf('/')+1)+'index.html?<xsl:value-of select="name"/>');
                    return false
                </xsl:attribute>
					[PLAY]
            </a>
